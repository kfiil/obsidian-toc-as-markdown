...
This is the **TOC as Markdown** Obsidian plugin - a utility that converts document headers into formatted table of contents as markdown within Obsidian notes.

### Core Plugin Architecture

#### Main Plugin Class: `TocAsMarkdownPlugin`
- **Lifecycle Management**: Handles `onload`, `onunload`, settings persistence
- **Header Processing Engine**: Core logic for detecting, parsing, and organizing document headers
- **Command Registration**: Editor commands for TOC generation and management
- **Markdown Generation**: Converts hierarchical header structure to formatted markdown TOC

#### Settings System: `TocAsMarkdownSettings`
- **Format Options**: TOC style configuration (bullets, numbers, mixed)
- **Indentation Control**: Spacing and nesting level preferences
- **Link Generation**: Anchor link format and behavior settings
- **Insertion Preferences**: Default TOC placement and update behavior

### TOC Processing Engine

#### Header Detection System
- **Markdown Headers**: Parses `#`, `##`, `###`, etc. with proper level recognition
- **Level Hierarchy**: Maintains proper nesting structure based on header depth
- **Content Extraction**: Extracts clean header text, removing formatting
- **Anchor Generation**: Creates URL-safe anchor links for internal navigation
- **Duplicate Handling**: Manages duplicate header names with unique identifiers

#### Link Processing
- **Internal Links**: Generates `[Header Text](#anchor-link)` format
- **Anchor Sanitization**: Converts headers to valid URL anchors
- **Case Handling**: Proper lowercase conversion and special character removal
- **Collision Resolution**: Ensures unique anchors for duplicate header names

### TOC Generation Engine

#### Format Rendering
- **Bullet Lists**: Generates `- [Header](#anchor)` hierarchical structure
- **Numbered Lists**: Creates `1. [Header](#anchor)` with proper numbering
- **Mixed Formatting**: Supports bullets for top-level, numbers for sub-levels
- **Indentation Logic**: Maintains consistent spacing for nested levels

#### Markdown Output
- **List Nesting**: Proper indentation for multi-level hierarchies
- **Link Formatting**: Consistent internal link generation
- **Spacing Control**: Configurable whitespace and line breaks
- **Header Filtering**: Optional inclusion/exclusion of specific header levels

### Key Features Implementation

#### TOC Generation System
- **Document Parsing**: `parseHeaders()` analyzes current document structure
- **Hierarchy Building**: Constructs nested TOC data structure from headers
- **Format Application**: Applies user-selected formatting to TOC structure
- **Position Control**: Inserts TOC at cursor position or configured location

#### Dynamic Updates
- **Live Refresh**: Updates TOC when document headers change
- **Incremental Updates**: Only refreshes changed sections for performance
- **Conflict Detection**: Handles manual TOC modifications gracefully
- **Version Tracking**: Maintains TOC consistency across document edits

#### Integration Features
- **Cursor Integration**: Places TOC at current cursor position
- **Selection Handling**: Replaces selected text with generated TOC
- **Document Boundaries**: Respects document structure and existing content
- **Undo Support**: Proper integration with Obsidian's undo system

### User Interface Components

#### Command Palette Integration
- **"Generate TOC"**: Creates new table of contents at cursor
- **"Update TOC"**: Refreshes existing TOC sections in document
- **"Remove TOC"**: Cleanly removes generated TOC markers
- **"TOC Settings"**: Quick access to plugin configuration

#### Settings Interface
- **Format Selection**: Radio buttons for TOC style preferences
- **Level Configuration**: Checkboxes for included header levels
- **Link Options**: Toggle for anchor link generation
- **Preview Display**: Live preview of TOC formatting

### Development Patterns

#### State Management
- **Settings Persistence**: Automatic save/load with `loadData()`/`saveData()`
- **Format Preferences**: User-selected TOC styling and behavior
- **Document State**: Tracking of TOC locations and update status
- **Cache Management**: Efficient header parsing with change detection

#### Error Handling
- **Invalid Headers**: Graceful handling of malformed markdown headers
- **Empty Documents**: Appropriate user feedback for headerless documents
- **Parse Failures**: Robust recovery from markdown parsing errors
- **Link Conflicts**: Resolution of duplicate or invalid anchor names

#### Performance Optimization
- **Lazy Parsing**: Headers parsed only when TOC generation requested
- **Change Detection**: Incremental updates for large documents
- **Memory Efficiency**: Minimal state retention between operations
- **Regex Optimization**: Compiled patterns for header detection

### Code Organization

#### Interface Definitions
- **TocAsMarkdownSettings**: Plugin configuration and user preferences
- **HeaderEntry**: Individual header data with level, text, and anchor
- **TocStructure**: Hierarchical representation of document outline
- **FormatOptions**: TOC rendering configuration and style settings

#### Method Responsibilities
- **Header Extraction**: `extractHeaders()` finds and parses document headers
- **TOC Generation**: `generateTocMarkdown()` creates formatted markdown output
- **Anchor Processing**: `createAnchorLinks()` generates unique internal links
- **Document Integration**: `insertTocAtPosition()` places TOC in editor
- **Update Management**: `refreshExistingToc()` maintains TOC synchronization

#### Processing Pipeline
- **Parse Phase**: Document analysis and header extraction
- **Structure Phase**: Hierarchy building and nesting organization  
- **Format Phase**: Markdown generation with user preferences
- **Insert Phase**: Document integration and cursor management

This architecture provides a clean, efficient solution for generating and maintaining markdown table of contents within Obsidian documents, with robust header detection and flexible formatting options.
